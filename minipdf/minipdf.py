...
class PDFName(PDFObject):
    ...
    def __str__(self):
        ...
        delimiters = "()<>[]{}/%"
        badchars = whitespaces + delimiters + "#"
        result = "/"
        for c in self.s:
            if c in badchars:
                result += "#%02x" % ord(c)
...
class PDFRef(PDFObject):
    ...
    def __str__(self):
        if self.obj[0].n is None:
            raise Exception(
                "Cannot take a reference of "
                + str(self.obj[0])
                + "because it is not added to any PDF Document"
            )
...
class PDFDoc(list):
    ...
    def append(self, obj):
        ...
        if obj.v != None or obj.n != None:
            raise Exception(
                "Object " + repr(obj) + " has been already added to a PDF Document!"
            )
        obj.v = 0
        obj.n = 1 + len(self)
